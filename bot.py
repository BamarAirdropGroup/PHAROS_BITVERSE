...
class Bitverse:
    ...
    def generate_order_payload(self, trade_pair: str, acceptable_price: int, trade_side: int):
        payload = {
            ...
            "leverageE2": 500,
            "side": trade_side,
            "margin": [{"denom": "USDT", "amount": str(int(self.trade_amount * 10**6))}],
            "allowedSlippage": "10",
            "isV2": "0"
        }
        return payload
    ...
    async def check_connection(self, proxy_url=None):
        ...
        try:
            ...
            self.log(
                ...
                f"{Fore.RED+Style.BRIGHT} Connection Failed - HTTP {e.status}: {e.message} {Style.RESET_ALL}"
            )
            return False
        except Exception as e:
            self.log(
            ...
            try:
                async with ClientSession(connector=connector, timeout=ClientTimeout(total=60)) as session:
                    async with session.post(url=url, headers=headers, data=data, proxy=proxy, proxy_auth=proxy_auth) as response:
                        ...
                        self.log(
                            f"{Fore.CYAN+Style.BRIGHT}   Status  :{Style.RESET_ALL}"
                            f"{Fore.GREEN+Style.BRIGHT} Successfully Fetched Balance {Style.RESET_ALL}"
                        )
                        return result
            except ClientResponseError as e:
                if e.status == 429:  # Rate limit
                    self.log(
                        f"{Fore.CYAN+Style.BRIGHT}   Status  :{Style.RESET_ALL}"
                        f"{Fore.YELLOW+Style.BRIGHT} Rate Limit Exceeded, Retrying in {2 ** attempt} seconds {Style.RESET_ALL}"
                    )
                    await asyncio.sleep(2 ** attempt)
                    continue
                self.log(
                    f"{Fore.CYAN+Style.BRIGHT}   Status  :{Style.RESET_ALL}"
                    f"{Fore.RED+Style.BRIGHT} HTTP Error {e.status}: {e.message} {Style.RESET_ALL}"
                )
                if attempt < retries - 1:
                    await asyncio.sleep(2 ** attempt)
                    continue
    ...
    async def order_simulation(self, address: str, trade_pair: str, acceptable_price: int, trade_side: int, use_proxy: bool, retries=5):
        ...
        self.log(
            f"{Fore.CYAN+Style.BRIGHT}   Payload :{Style.RESET_ALL}"
            f"{Fore.YELLOW+Style.BRIGHT} {data} {Style.RESET_ALL}"
        )
        for attempt in range(retries):
            proxy_url = self.get_next_proxy_for_account(address) if use_proxy else None
            connector, proxy, proxy_auth = self.build_proxy_config(proxy_url)
                ...
                async with ClientSession(connector=connector, timeout=ClientTimeout(total=60)) as session:
                    async with session.post(url=url, headers=headers, data=data, proxy=proxy, proxy_auth=proxy_auth) as response:
                        ...
                        self.log(
                            ...
                            f"{Fore.YELLOW+Style.BRIGHT} HTTP {response.status}: {response_text} {Style.RESET_ALL}"
                        )
                        response.raise_for_status()
                        result = json.loads(response_text)
                        self.log(
                            f"{Fore.CYAN+Style.BRIGHT}   Status  :{Style.RESET_ALL}"
                            f"{Fore.GREEN+Style.BRIGHT} Successfully Built Order Simulation {Style.RESET_ALL}"
                        )
                        return result
            except ClientResponseError as e:
                if e.status == 429:  # Rate limit
    ...
    async def process_check_connection(self, address: str, use_proxy: bool, rotate_proxy: bool):
        while True:
            ...
            if not is_valid and rotate_proxy and use_proxy:
